...
import './Share.css'; // ตรวจสอบว่า path ถูกต้อง
...
const ShareItemCard = ({ item, onInterested }) => {
  ...
  const handleRequestClick = () => {
    if (!isRequested) {
      setIsRequested(true);
      onInterested(item.shareId); // ใช้ shareId เป็น identifier
    }
  };
...
const RequestList = ({ requests , itemId, onApproveShare}) => {
...
const MySharesTab = ({ items, onAddNewShare, onApproveShare }) => {
  const [viewingRequestsFor, setViewingRequestsFor] = useState(null);
  const handleViewRequests = (itemId) => {
    setViewingRequestsFor(prev => (prev === itemId ? null : itemId));
  };

  if (!items || items.length === 0) {
    return (
      <div className="tab-content empty-my-shares">
        <p className="empty-state-message larger-empty-message">ไม่มีรายการแบ่งปันของคุณ</p>
    ...
    <div className="tab-content">
      {items.map(item => (
        <React.Fragment key={item.id}>
          <MyShareItemCard item={item} onViewRequests={handleViewRequests} />
          {viewingRequestsFor === item.id && (
            <RequestList requests={dummyRequests[item.id] || []} 
            itemId={item.id}
...
function ShareScreen({ mySharesData, onApproveShare }) { // รับ mySharesData จาก props
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setShares(allSharesData); // ✅ เก็บใน state
  ...
  return (
    <div className="share-screen-container">
      ...
      <div className="search-bar-container-share">
        <div className="search-bar-share"> {/* กรอบนอกสุด */}
          <FaSearch className="search-icon-share" /> {/* ไอคอนแว่นขยาย */}
          <div className="inner-input-wrapper"> {/* <<< เพิ่ม wrapper นี้เข้ามา <<< */}
            <input
              type="text"
              placeholder="ค้นหาวัตถุดิบหรืออาหารที่ต้องการ"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            {searchTerm && <FaTimes className="clear-icon-share" onClick={() => setSearchTerm('')} />}
          </div>
        </div>
      </div>
